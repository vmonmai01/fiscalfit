<div class="bg-claro p-5 rounded-lg m-2">
    ...
    <div class="relative rounded-lg mx-5 my-5">
        ...
        <form class="max-w-lg mx-auto py-5 px-5">
            <div class="flex">
                ...
                <div class="relative w-full">
                    <input type="text" id="search" wire:model.live="expenseSearch"
            ...
            <tbody>
                @forelse($expenses as $expense)
                    <tr class="border-b bg-oscuro border-gray-700 hover:bg-gray-600">
                        <td class="px-6 py-4"> <input id="{{ $expense->id }}" type="checkbox"
                                wire:model="selectedExpenses"
                        ...
                        <td class="px-6 py-4 flex justify-center ">
                            @if ($expense->photo != null)
                                <a href="{{ asset('storage/expenses_photos/' . $expense->photo) }}" target="_blank">
                                    <img src="{{ asset('storage/expenses_photos/' . $expense->photo) }}"
                                        alt="Imagen del Gasto {{ $expense->id }}" width="50" height="50">
                                </a>
                            @else
                ...
                @empty
                    <tr class="border-b bg-oscuro border-gray-700 hover:bg-gray-600">
                        <td colspan="6" class="text-red-500 text-center font-bold py-4"> No se encontraron resultados
                            para la búsqueda:
                            "{{ $expenseSearch }}" </td>
                    </tr>
                @endforelse
        ...
        <button class="bg-red-700 hover:bg-red-800 text-white font-bold py-2 px-4 rounded"
            wire:click="deleteSelected">Eliminar Seleccionados</button>
